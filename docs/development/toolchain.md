...
この文書は、Accepted ADR-001、ADR-002、ADR-004、ADR-006、ADR-007、ADR-009に基づく初回Sliceの再現可能なtoolchain基準です。versionは2026-09-20時点の最新安定版として固定します。更新は依存関係ポリシーとADR-007のレビュー手順に従って行います。
...
OIDC client/token verification用のGo libraryは未選定であり、実装前に別途依存関係評価を行う。未承認ライブラリを追加してはならない。
